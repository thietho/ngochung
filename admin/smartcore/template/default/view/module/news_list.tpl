        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <div class="pull-left margin-bottom-20">
                    <button onclick="window.location.href='<?php echo HTTP_WEB ?>?route=module/news/insert'" type="button" class="btn btn-default btn-bg btn-success"><span class="fa fa-plus"></span> Add news</button>
                </div>
                <?php } ?>
...
<script type="application/javascript">
    function News()
    {
        ...
        {
            showLoading();
            $.get("?route=module/news/loadData", $('#frmNewsSearch').serialize(), function(html){
                endLoading();
                $('#news-list').html(html);
            });
        };
        ...
        {
            $.get('?route=module/news/delete&id=' + this.id,function(data){
                if(data == 'true')
                {
    ...
    $('#frmNewsSearch #sitemapid').val("<?php echo $sitemapid?>");
