function goPage(page, filter) {
    window.location.href = "?route=core/addon/" + filter + "&page=" + page;
}

function confirmDeleteList() {
    showSimpleModal("<?php echo $lang_text_delete_data ?>", "<?php echo $lang_text_confirm_delete ?>", "addon_delete()", "<?php echo $lang_text_cancel?>", "<?php echo $lang_text_delete ?>");
}
...
function addon_delete() {
    ...
    var url = "?route=core/addon/deleteList";
...
function updateList() {
    ...
    var $url = "index.php?route=core/addon/updatePosition";
    $.post($url, $('#formlist').serialize(), function ($data) {
        var $response = JSON.parse($data);
        var $errors = $response['errors'];
        if($response['status'] == 'failed') {
