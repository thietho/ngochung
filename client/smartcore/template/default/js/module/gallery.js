function goPage(page, filter) {
    window.location.href = "?route=module/gallery/" + filter + "&page=" + page;
}
...
function updateList() {
    ...
    var $url = "index.php?route=module/gallery/updatePosition";
...
function confirmDeleteList() {
    showSimpleModal("<?php echo $lang_text_delete_data ?>", "<?php echo $lang_text_confirm_delete ?>", "gallery_delete()", "<?php echo $lang_text_cancel?>", "<?php echo $lang_text_delete ?>");
}
...
function gallery_delete() {
    ...
    var url = "?route=module/gallery/deleteList";
    $.post(url, $('#formlist').serialize(), function ($data) {
        var $response = JSON.parse($data);
        var $errors = $response['errors'];
        if($response['status'] == 'failed') {
...
function publish(onwer, $mediaid, $status) {
    ...
    $.post("<?php echo HTTP_SERVER ?>?route=module/gallery/active", {mediaid : $mediaid, status : $status},function($data) {
        endloading();
...
function sort(object, $orderby) {
    ...
    window.location.href = "?route=module/gallery/&sitemapid="+ $sitemapid + "&orderby=" + $orderby + "&sort=" + $sort + "&page=" + $page;
}
...
function searchForm() {
    ...
    var url = "?route=module/gallery&sitemapid=" + $sitemapid;
