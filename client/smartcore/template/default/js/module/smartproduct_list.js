...
function goPage(page, filter) {
    window.location.href = "?route=module/smartproduct/" + filter + "&page=" + page;
}
...
function updateList() {
    ...
    var $url = "index.php?route=module/smartproduct/updatePosition";
...
function product_delete() {
    ...
    var url = "?route=module/smartproduct/deleteList";
    $.post(url, $('#formlist').serialize(), function ($data) {
        var $response = JSON.parse($data);
        var $errors = $response['errors'];
        if ($response['status'] == 'failed') {
...
function sort(object, $orderby) {
    ...
    window.location.href = "?route=module/smartproduct/&sitemapid=" + $sitemapid + "&orderby=" + $orderby + "&sort=" + $sort + "&page=" + $page;
}
...
function publishProduct(onwer, $model, $status) {
    ...
    $.post("<?php echo HTTP_SERVER ?>?route=module/smartproduct/active", {model : $model, status : $status},function($data) {
        endloading();
...
function searchForm() {
    ...
    var url = "?route=module/smartproduct&sitemapid=" + $sitemapid;
...
function viewProductAll() {
    var $sitemapid = "<?php echo $sitemap['sitemapid'] ?>";
    window.location.href = "?route=module/smartproduct&sitemapid=" + $sitemapid;
}
