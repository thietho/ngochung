...
if (!defined('__CSRF_PROTECTOR__')) {
	...
	class csrfProtector
	{
		...
		public static function init($length = null, $action = null)
		{
			 ...
			 */
			$standard_config_location = __DIR__ ."/csrf_config.php";
			$composer_config_location = __DIR__ ."csrf_config.php";
			...
			foreach (self::$requiredConfigurations as $value) {
				if (!isset(self::$config[$value]) || self::$config[$value] == '') {
                    ...
                    //('logDirectory', 'failedAuthAction', 'jsPath', 'jsUrl', 'tokenLength');
